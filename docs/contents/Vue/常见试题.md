...
**$refs**
* 优点：直接访问子组件实例
* 缺点：只在子组件渲染完成才可获取，不能用于computed中
...
component 是一个可复用组件的集合，它包括了html、js、css 的合集
...
extend是一个构造器，他和component类似都是创造一个组件的，不过component首先要创建一个.vue文件，而extend可以直接在js文件中调用，可以用于动态创建组件